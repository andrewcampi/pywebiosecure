...
def start_server(applications, port=8080, host='', cdn=True,
                 ...
                 session_expire_seconds=None,
                 session_cleanup_interval=None,
                 debug=False, max_payload_size='200M', byoc=[], **django_options):
    ...
    if len(byoc) < 2:
        # Generate a new cert
        output = subprocess.getoutput('openssl req -newkey rsa:2048 -new -nodes -x509 -days 3650 -keyout key.pem -out cert.pem -subj "/C=US/ST=New-Jersey/L=Manasquan/O=PywebioSecure/OU=IT/CN=PywebioSecureApplication"')
        print("Generated a new cert.")
        ssl_options = ["cert.pem", "key.pem"]
        if use_tornado_wsgi:
            import tornado.wsgi
            container = tornado.wsgi.WSGIContainer(app)
            http_server = tornado.httpserver.HTTPServer(container, ssl_options={"certfile":ssl_options[0], "keyfile":ssl_options[1]}, max_buffer_size=max_payload_size)
